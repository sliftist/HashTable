	...
	if(self->currentAllocation.valueForSet) {
		...
		if(!table->newAllocation.valueForSet) {
			...
			while(true) {
				...
				if(!value.isNull) {
					// This is actually... not dangerous. Reference_GetHash doesn't need to access any user data, it only
					//  nees to access the InsideReference, which is always inside our allocation, so this is safe.
